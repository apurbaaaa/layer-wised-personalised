...
\title[FedSA-Drift]{FedSA-Drift: Structure- and Drift-Aware Federated Vision Transformers for Multimodal Skin Lesion Classification}
...
\begin{abstract}
\sloppy
Federated learning allows healthcare institutions to train shared machine learning models on patient records without exchanging raw data, supporting compliance with privacy regulations such as GDPR and HIPAA. Heterogeneity across institutions, however, induces client drift, unstable convergence and unfair per-client performance, and these effects intensify when structured clinical metadata is fused with high-capacity Vision Transformers. This paper proposes \sadrift{} (Structure- and Drift-Aware Federated Learning) for multimodal skin lesion classification. A SwinV2-Base backbone fused with a metadata branch is separated into three parameter groups---early vision layers, deeper Transformer layers, and client-specific multimodal heads---each aggregated by a different rule: standard FedAvg, drift-aware inverse-distance weighting, and no aggregation, respectively. All correction occurs at the server during aggregation: no penalty term is added to any local objective and no regularisation hyperparameter is introduced. Our central result is obtained on the held-out ISIC 2019 test collection under severe heterogeneity ($\alpha = 0.1$, $K = 5$), where one client holds no samples for four of the eight classes. There, standard FedAvg attains the highest mean per-client accuracy (0.4702) yet abandons four of the eight diagnostic classes outright, recording \emph{zero} melanoma recall for every client---including the institution holding 3815 of the federation's 3844 melanoma cases (0/1327, 95\% Wilson interval [0.000, 0.003])---because size-weighted aggregation is dominated by a client that holds none. Structure-aware decomposition restores all eight classes at a cost of 15.9 percentage points of mean accuracy, and drift-aware weighting recovers 11.5 of those points while raising the worst-client floor by 21.5 points and reducing the inter-client standard deviation of accuracy by 67\%. At the melanoma-holding client, recall rises from 0.000 to 0.573 with the decomposition and to 0.727 with drift weighting, across three disjoint confidence intervals. Balanced accuracy separates these very different clinical profiles by roughly one point (0.3778 against 0.3681): aggregate metrics do not reveal complete class abandonment. At the milder settings, worst-client accuracy rises from 0.4800 to 0.5705 at $\alpha = 0.5$ with a 39\% reduction in inter-client standard deviation. These proof-of-concept results, spanning three federated configurations on a single dataset, suggest improved class coverage, stability and fairness.
...
\begin{itemize}
    ...
    \item \textbf{Low Computational Overhead:} The server-side cost of drift-aware aggregation is negligible relative to local training (Section~\ref{sec:budget}), and clients are unmodified, storing no auxiliary state. We measured computational and communication cost only; we did not evaluate deployment, which would additionally involve network conditions, edge hardware and institutional integration.
...
\textbf{Partial personalization.} FedPer~\citep{Collins2021} and FedRep~\citep{Collins2021} keep a classification head local and share a representation; FedBABU trains the body with the head frozen and personalizes only at the end; FedBN keeps normalisation statistics local. \sadrift{}'s Group~C is the same idea as FedPer's private head, extended to a multimodal head that also carries the metadata branch. The difference from all four is not the existence of a private head but that the \emph{shared} portion is itself split, with early and deep layers aggregated by different rules---FedPer, FedRep, FedBABU and FedBN all apply one aggregation rule to everything they share.
...
\begin{algorithm}[t]
\caption{\sadrift{}: Structure-Aware Federated Training with Drift-Aware Aggregation}
\label{alg:federated}
\begin{algorithmic}[1]
...
\sadrift{} achieves a lower global balanced accuracy than the uniform-aggregation baseline at the milder settings (0.8209 vs.\ 0.8314 at $\alpha{=}1.0$; 0.7066 vs.\ 0.7622 at $\alpha{=}0.5$). The more pronounced gap at $\alpha{=}0.5$ (5.6 percentage points, vs.\ 1.1 at $\alpha{=}1.0$) reflects a genuine cost of the fairness--utility trade-off that is well-recognized in the federated learning literature~\citep{Li2021,Mohri2019}. We do not claim this cost is negligible; a global accuracy reduction of this magnitude in medical imaging warrants careful clinical evaluation before deployment.
...
\small
\setlength{\tabcolsep}{3pt}
\begin{tabular}{p{3.8cm}p{2.2cm}p{2.0cm}p{2.0cm}p{3.0cm}p{2.6cm}}
\toprule
...
The ISIC 2019 dataset analyzed in this study is publicly available from the International Skin Imaging Collaboration (ISIC) Archive at \url{https://challenge.isic-archive.com/data/}. All code required to reproduce the reported experiments---model definition, partitioning procedure, aggregation rules, ablation arms and evaluation scripts---is publicly available at \url{https://github.com/apurbaaaa/layer-wised-personalised}.
